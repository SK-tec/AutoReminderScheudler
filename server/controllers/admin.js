...
const login = async (req, res) => {
  ...
  if (!adminDocument) {
    ...
    try {
      const isPasswordValid = await bcrypt.compare(req.body.password, adminDocument.password);
      if (!isPasswordValid) {
        res.status(400).json({ message: 'Invalid Login Attempt' });
      } else {
        const payload = {
...
const getLoggedInUser = async (req, res) => {
  try {
    const getAdmin = await Admin.findOne({ _id: req.user._id }).select('-password');
    res.json(getAdmin);
  } catch (error) {
